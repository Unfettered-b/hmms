#!/bin/bash
# =============================================================================
# ESCRT system proteins Search - Large Proteome
...
mkdir -p "$SOURCES_DIR"
ASCOG_TSV="$SOURCES_DIR/ascogs_escrt_system_20260116_121403.tsv"
AFA_DIR="$SOURCES_DIR/asCOG.ali/ali.8"

echo "↓ Preparing ESCRT system HMMs..."
ESCRT_HMM="$SOURCES_DIR/ESCRT_system_iter3.hmm"
...
if [ ! -f "$ESCRT_HMM" ]; then
    ...
    mapfile -t ASCOG_IDS < <(cut -f1 "$ASCOG_TSV")
    ...
    hmmpress "$ESCRT_HMM"
    echo "✓ ${ESCRT_HMM} ready"
else
    echo "✓ ${ESCRT_HMM} already exists"
fi
...
echo "ESCRT system HMMs  : ${#ASCOG_IDS[@]}"
